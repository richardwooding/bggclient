...
BoardGameGeek XML API 1 client, shipped three ways: a kong-based CLI (`shelfofshame`), an MCP server (`shelfofshame serve`, official MCP Go SDK), and a Go library (`xml1` package). Module: `github.com/richardwooding/shelfofshame`, Go 1.26.
...
**Trademark constraint**: BoardGameGeek® and BGG® are registered trademarks of BoardGameGeek, LLC — never use "bgg" or "boardgamegeek" in product names, binary names, repo names, tool names, or other branding (the project was renamed from bggclient for this reason). Descriptive references to the BoardGameGeek XML API in prose are nominative fair use and fine; env vars like `BGG_API_TOKEN` name BGG's own token and stay. README and site must keep the "Powered by BGG" logo (required by the API ToU) and the non-affiliation disclaimer.
...
- `internal/gameopts/` — flag/input structs shared by CLI and MCP, tri-tagged (kong `help:` / `json:` / `jsonschema:`), with `Options()` converters to `xml1` functional options. Note naming traps in xml1: `Comments` (boardgame) vs `Comment` (collection) vs `GeeklistComments` (presence-only var).
- `internal/mcpserver/` — `New(api, version)` returns an `*mcp.Server` with four tools (`search_boardgames`, `get_boardgames`, `get_collection`, `get_geeklist`). Handlers use the generic `mcp.AddTool` typed pattern; output structs are the `xml1/model` types directly (they carry json tags for this reason). Transports: stdio (default) and streamable HTTP (`serve --http`).
...
Tag `v*` on main → `.github/workflows/release.yml` runs goreleaser: GitHub release with archives, Homebrew cask pushed to `richardwooding/homebrew-tap` (needs `HOMEBREW_TAP_GITHUB_TOKEN` secret), and a ko-built OCI image at `ghcr.io/richardwooding/shelfofshame`. The gloam site in `docs/` deploys to GitHub Pages via `pages.yml`; gloam assets are vendored and refreshed by `gloam-sync.yml`.
